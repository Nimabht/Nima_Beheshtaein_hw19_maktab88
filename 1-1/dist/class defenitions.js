"use strict";
class RaceCar {
    constructor(name, orderInGame = 0) {
        this.name = name;
...
class Game {
    ...
    makeGameRules() {
        ...
        if (!!carCountFromClient)
            this.carCount = +carCountFromClient;
        ...
        for (let i = 0; i < this.carCount; i++) {
            ...
            if (!!carName)
                this.cars.push(new RaceCar(carName, orders[i] + 1));
        }
    ...
    mapSimulator() {
        ...
        ranks = ranks.map((rank, index) => `Rank ${index + 1}: ${rank} ${index === 0 ? "(Winner!!!)" : ""}\n`);
